...
box_list_t *box_before, *box_after; //全局bounding box链表
box_trans_t bounding_boxes[SS*SS*B]; //全局boudnding box
...
void dequantization(float *dest, signed char *src)
{
    for(int i = 0; i < SS; i++)
    {
        for(int j = 0; j < SS; j++)
        {
            ...
            {
                dest[i*SS*(5*B+C)+j*(5*B+C)+k] = src[i*SS*(5*B+C)+j*(5*B+C)+k] * SCALE_FACTOR;
            }
        }
    }
}
...
int list_insert(box_list_t *phead, box_trans_t *box)
{
    ...
    box_list_t *item = (box_list_t *)malloc(sizeof(box_trans_t));
    if(item)
    {
        item->data = box; 
        item->next = NULL;
...
int list_delete(box_list_t *phead, box_trans_t *box)
{
    ...
    else //temp->next->data == box
    {
        ...
        //free(pre->next); //不能回收内存，否则L169 null pointer
        pre->next = nxt;
